...
const (
	// DefaultUserAgent .
	DefaultUserAgent = "Go Gb28181/1.0.0"
)
...
func NewSipStack(config *SipStackConfig) *SipStack {
	...
	logger := utils.NewLogrusLogger(config.LogLevel, "SipStack", nil)
	...
	s.log = logger
	s.tp = transport.NewLayer(ip, dnsResolver, config.MsgMapper, utils.NewLogrusLogger(config.LogLevel, "transport.Layer", nil))
	sipTp := &sipTransport{
		tpl: s.tp,
		s:   s,
	}
	s.tx = transaction.NewLayer(sipTp, utils.NewLogrusLogger(config.LogLevel, "transaction.Layer", nil))
...
func (s *SipStack) handleRequest(req sip.Request, tx sip.ServerTransaction) {
	...
	logger.Infof("routing incoming SIP request:\n%s", req)
...
func (s *SipStack) AckInviteRequest(request sip.Request, response sip.Response, body string) {
	ackRequest := sip.NewAckRequest("", request, response, body, log.Fields{
		"sent_at": time.Now(),
	})
